...
| [Unified MCP](/mcp/overview) | AI agents (Cursor, Claude, ChatGPT) | See [Connect](/mcp/connect) — CLI header / Claude OAuth / ChatGPT `?api-key=` |
| [Claude](/integrations/claude) | Claude.ai (OAuth) + Claude Code (header) | Web `/web/mcp` OAuth; CLI `/mcp` + `X-API-Key` |
...
<CardGroup cols={2}>
  ...
  <Card title="Claude" icon="robot" href="/integrations/claude">
    Claude.ai OAuth on `/web/mcp` · Claude Code API key on `/mcp`.
  </Card>

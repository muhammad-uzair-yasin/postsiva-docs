...
This guide walks you from **zero → connected social accounts → API key → your first automated post**. Use the **web app** for connect + keys (recommended), then use the **REST API** or **MCP** for everything else.

```mermaid
flowchart TD
  A[Sign up / log in] --> B[Create or join workspace]
  B --> C[Upgrade to Pro]
  C --> D[Connect social accounts]
  D --> E[Create workspace API key]
  E --> F[Upload media / post / analytics]
  D -.->|or| G[POST /unified/oauth/url]
  G --> D
  E --> H[REST or MCP]
  H --> F
```
...
### 1. Sign up and open a workspace

1. Create an account (email OTP + password) or log in.
2. Create a **workspace** (or accept an invite).
3. Confirm you are in the correct workspace (top / workspace switcher). All OAuth tokens and posts are scoped to that workspace.
...
2. Use endpoint: `https://mcp.postsiva.com/mcp`
3. Header: `X-API-Key: psk_live_YOUR_KEY`
4. Follow [Connect MCP](/mcp/connect) and [Cursor](/mcp/cursor).

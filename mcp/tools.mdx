...
| Server | URL | Upload | Publish |
|--------|-----|--------|---------|
| CLI | `/mcp` | `upload_media` + local `file_path` → curl | JSON `publish` (immediate) |
| Web | `/web/mcp` | `upload_media` iframe + `complete_media_upload` | **Preview App** — account toggles, then Post / Schedule / Draft / Reject |

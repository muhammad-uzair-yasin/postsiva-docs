...
| `complete_media_upload` | Web only | Confirm widget upload → `media_id` / `public_url` for publish |
...
| `content_to_image` | Post text → generated images |

None of the AI tools publish automatically — chain with `upload_media` / `publish` if needed.
